...
if [ -f "/etc/nginx/sites-available/djmanager" ] && [ -f "\$_ENV_FILE_NG" ]; then
  ...
  _NG_SN="\$(echo "\${_NG_DOMAIN:-_}" | tr ',' ' ')"
  {
    ...
    printf '\n'
    printf '    # Security headers are set by Django'\''s SecurityHeadersMiddleware.\n'
    printf '    # Setting them here too would cause duplicate headers (ZAP/scanner warning).\n'
    printf '\n'
    printf '    gzip on;\n'
    printf '    gzip_vary on;\n'
    printf '    gzip_proxied any;\n'
    printf '    gzip_comp_level 6;\n'
    printf '    gzip_types text/plain text/css text/xml application/json application/javascript\n'
    printf '               text/javascript application/x-javascript image/svg+xml;\n'
    printf '\n'
    ...
    printf '        alias %s/staticfiles/;\n' "\$MANAGER_DIR"
    printf '        expires 1y;\n'
    printf '        access_log off;\n'
    printf '        add_header Cache-Control "public, immutable";\n'
    printf '        error_page 404 = @mgr_static_fallback;\n'
    printf '    }\n\n'
    printf '    # Static fallback: files not yet in staticfiles/ served via WhiteNoise.\n'
    printf '    location @mgr_static_fallback {\n'
    ...
    printf '    }\n\n'
    printf '    # Job-poll endpoint: low-cost in-memory lookup — no rate limiting needed.\n'
    printf '    location /jobs/ {\n'
    printf '        proxy_pass http://%s:%s;\n' "\${_NG_IP}" "\${_NG_PORT}"
    printf '        proxy_set_header Host \$http_host;\n'
    printf '        proxy_set_header X-Real-IP \$remote_addr;\n'
    printf '        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;\n'
...
# Cloudflare Real-IP Config aktuell halten (für bestehende Installationen)
...
real_ip_recursive on;
CFREALIP
echo "✅ Cloudflare Real-IP-Config aktualisiert"
...
server {
    ...
    # A static nginx CSP here would conflict (browser applies both headers, nonce scripts blocked).
    add_header Strict-Transport-Security "max-age=31536000; includeSubDomains" always;
    add_header Cross-Origin-Opener-Policy "same-origin" always;
    add_header Cross-Origin-Embedder-Policy "unsafe-none" always;
    add_header Cross-Origin-Resource-Policy "same-origin" always;
